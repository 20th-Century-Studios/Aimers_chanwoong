...
for fold in range(5):
    ...
    # Predict on the validation set (-1 for outliers, 0 and above for inliers)
    y_pred_valid = model.fit_predict(X_valid)
    
    # Map DBSCAN output to binary class labels (-1 is an outlier, 1 is an inlier)
...
test_predictions = model.fit_predict(X_test_scaled)

# Map DBSCAN output to binary class labels for test data
...
output.to_csv('../output/submission_DBSCAN.csv', index=False)
